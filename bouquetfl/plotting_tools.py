...
results["total_train_time"] = [results["train_time_1"][i] + results["train_time_2"][i] + results["train_time_2"][i] + results["train_time_2"][i] for i in range(len(results["id"]))]
results["total_load_time"] = [results["load_time_1"][i] + results["load_time_2"][i] + results["load_time_3"][i] + results["load_time_4"][i] for i in range(len(results["id"]))]
print(results)
...
def gpu_times(df):
    ...
    for i in range(len(df["train_time_1"])):
        ax.barh(i,  df["train_time_1"][i], .4, left = 0.001, color = "royalblue")
...
def cpu_times(df):
    ...
    for i in range(len(df["load_time_1"])):
        ax.barh(i-.2,  df["load_time_1"][i], .4, left = 0.001, color = "royalblue")
...
def plot_federation_timeline(df):
    ...
    for round in range(1, 5):
        ...
        for client in range(len(df["id"])):
            ax.barh(client,  df[f"load_time_{round}"][client], .4, left = starttime, color = "red")
            ax.barh(client,  df[f"train_time_{round}"][client], .4, left = starttime + df[f"load_time_{round}"][client], color = "green")
            if df[f"load_time_{round}"][client] + df[f"train_time_{round}"][client] > max_time:
                max_time = df[f"load_time_{round}"][client] + df[f"train_time_{round}"][client]
        if round <  4:
            plt.vlines(starttime + max_time + .05, ymin = -0.2, ymax = len(df["id"]) - 0.8, color = "black", linewidth=.1)
        starttime += max_time + .1
    y_ticks = [f"{df['gpu'][client]} \n {df['cpu'][client]}" for client in range(len(df["id"]))]
